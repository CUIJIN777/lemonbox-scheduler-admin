...
pipeline {
  environment {
    ...
    registry = "registry-vpc.cn-beijing.aliyuncs.com/lemonbox/lemonbox-scheduler-admin"
      ...
      steps {
        script {
          ...
          if (env.GIT_BRANCH == "origin/master") {
            ...
            sh "kubectl rollout status deployment/lemonbox-scheduler-admin"
          } else {
            sh "kubectl config use test"
            sh "cat k8s/deployment-test.yml | sed 's/{{GIT_COMMIT}}/$GIT_COMMIT/g' | kubectl apply -f -"
            sh "kubectl rollout status deployment/lemonbox-scheduler-admin-test"
          }
        }
      }
    }
    ...
    aborted {
      script {
        if (FAILED_STAGE == "Update k8s cluster") {
          if (env.GIT_BRANCH == "origin/master") {
            sh "kubectl config use 229638354516261836-c1ef3c2118ae344d3b5ee63c17ae6f93b"
            sh "kubectl rollout undo deployment/lemonbox-scheduler-admin"
          } else {
            sh "kubectl config use test"
            sh "kubectl rollout undo deployment/lemonbox-scheduler-admin-test"
          }
        }
      }
    }
